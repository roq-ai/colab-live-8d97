...
import { getStreamById, updateStreamById } from 'apiSdk/streams';
...
function StreamEditPage() {
  ...
  const handleSubmit = async (values: StreamInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateStreamById(id, values);
      mutate(updated);
      resetForm();
      router.push('/streams');
    } catch (error) {
  ...
  const formik = useFormik<StreamInterface>({
    initialValues: data,
